...
namespace DataRepresentor
{
    public class DataRocController : DataRepresentorController
    {
        ...
        public override void AddValueToLine(Candle candle, Func<Candle, decimal> funcOpen, Func<Candle, decimal> funcClose)
        {
            ...
            var candleValues = SeriesCollection[0].Values;
            Random rnd = new Random();
            var lineValues = SeriesCollection[1].Values;
            double value = 0;
            if (LastCandle != null)
            {
                int x = rnd.Next(0,3);
                if (x == 0 || x == 1 || x == 3)
                    value = LastCandle == null ? 0 : (double)(funcOpen.Invoke(LastCandle)) + 0.01;
                else
                    value = LastCandle == null ? 0 : (double)(funcClose.Invoke(LastCandle)) - 0.01;
